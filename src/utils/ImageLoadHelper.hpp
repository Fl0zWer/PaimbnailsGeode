...
namespace ImageLoadHelper {
     ...
     */
    inline LoadedImage createFromRGBA(const uint8_t* rgba, int w, int h) {
        LoadedImage result;
    ...
    inline LoadedImage loadWithSTB(std::filesystem::path const& path) {
        ...
        if (!result.success) {
            ...
            log::info("[ImageLoadHelper] Loaded via stb_image: {}x{} (original {} channels)", w, h, channels);
        }
        return result;
    }
    ...
    inline LoadedImage loadStaticImage(std::filesystem::path const& path, size_t maxSizeMB = 10) {
        ...
        // === Intento 2: leer bytes y usar CCImage::initWithImageData ===
        {
        ...
        // === Intento 3: stb_image fallback (BMP, TGA, PSD, JPEG especiales, etc) ===
        {
            auto stbResult = loadWithSTB(path);
