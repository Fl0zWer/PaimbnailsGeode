...
void ProfileReviewsPopup::buildReviewList(float average, int count, const matjson::Value& reviews) {
    ...
    if (reviews.isArray()) {
        auto arr = reviews.asArray().unwrap();
        for (auto& item : arr) {
            if (!item.isObject()) continue;
            std::string user = item["username"].isString() ? item["username"].asString().unwrap() : "???";
            int stars = item["stars"].isNumber() ? static_cast<int>(item["stars"].asInt().unwrap()) : 0;
            std::string msg = item["message"].isString() ? item["message"].asString().unwrap() : "";
    ...
    auto scroll = cocos2d::extension::CCScrollView::create();
    scroll->setViewSize({scrollW, scrollH});
    scroll->setPosition({scrollX, scrollY});
    scroll->setDirection(kCCScrollViewDirectionVertical);
    scroll->setContainer(container);
    scroll->setContentOffset({0.f, scrollH - totalH}, false);
