...
void PaimonShopPopup::fetchShopList() {
    HttpClient::get().getPetShopList([this](bool success, std::string const& response) {
        if (!success) {
            if (m_statusLabel) m_statusLabel->setString("Failed to load shop");
            return;
        }

        auto res = matjson::parse(response);
            if (!res.isOk()) {
                if (m_statusLabel) m_statusLabel->setString("Invalid response");
                return;
            }
            ...
            if (json.contains("items") && json["items"].isArray()) {
                ...
                if (m_statusLabel) m_statusLabel->setString("No pets available yet");
                return;
            }

            auto arrRes = arr.asArray();
            if (!arrRes.isOk()) {
                if (m_statusLabel) m_statusLabel->setString("Error parsing shop data");
                return;
            }
            for (auto& item : arrRes.unwrap()) {
                ...
                if (!p.id.empty()) m_items.push_back(p);
            }
...
void PaimonShopPopup::onDownload(CCObject* sender) {
    ...
    HttpClient::get().downloadPetShopItem(itemId, format,
        [this, itemId, format, name](bool success, std::vector<uint8_t> const& data) {
            m_downloading.erase(itemId);
            ...
            auto path = PetManager::get().galleryDir() / filename;
            {
                ...
                // rebuild list to show "Downloaded"
                buildList();
            }
        });
}
...
void PaimonShopPopup::onUploadPet(CCObject*) {
    ...
    auto filepath = std::filesystem::path(result.value());
    ...
    HttpClient::get().uploadPetShopItem(defaultName, creator, data, format,
        [this, defaultName](bool success, std::string const& response) {
            if (success) {
