...
void PaiConfigLayer::buildExtrasTab() {
    ...
    comingSoon->setPosition({cx, contentMid - 55});
...
void PaiConfigLayer::rebuildProfilePreview() {
    ...
    if (type != "custom" || path.empty() || !std::filesystem::exists(path)) {
    ...
    sprite->setPosition({thumbSize / 2, thumbSize / 2});
...
void PaiConfigLayer::rebuildBgPreview() {
    ...
    // ══════════════════════════════════════
    if (cfg.type == "custom") {
        if (cfg.customPath.empty() || !std::filesystem::exists(cfg.customPath)) {
    ...
    if (isLayerRef || cfg.type == "menu") {
        ...
        if (resolvedCfg.type == "custom" && !resolvedCfg.customPath.empty() && std::filesystem::exists(resolvedCfg.customPath)) {
            ...
            if (ext == ".gif") {
                ...
                AnimatedGIFSprite::createAsync(gifPath, [self, selectedKey, pw, ph, midX, midY, cfg](AnimatedGIFSprite* anim) {
                    ...
                    anim->setPosition({midX, midY});
                    anim->setAnchorPoint({0.5f, 0.5f});
                    auto stencil2 = CCLayerColor::create({255, 255, 255, 255});
                    stencil2->setContentSize({pw, ph});
...
void PaiConfigLayer::onBgCustomImage(CCObject*) {
    ...
    if (result.has_value() && !result.value().empty()) {
        auto pathStr = result.value().generic_string();
