...
void LeaderboardLayer::updateLevelInfo() {
    ...
    std::function<CCNode*(CCNode*, int)> findByTag = [&](CCNode* parent, int tag) -> CCNode* {
    ...
    // actualizar nombre
    if (auto nameLbl = typeinfo_cast<CCLabelBMFont*>(findByTag(container, TAG_NAME_LABEL))) {
        nameLbl->setString(m_featuredLevel->m_levelName.c_str());
    }

    // actualizar creador
    if (auto creatorLbl = typeinfo_cast<CCLabelBMFont*>(findByTag(container, TAG_CREATOR_LABEL))) {
