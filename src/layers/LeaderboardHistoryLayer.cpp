...
bool LeaderboardHistoryLayer::init() {
    ...
    // forzar BG silenciado cada frame
    ensureBgSilenced();
    this->scheduleUpdate();
...
void LeaderboardHistoryLayer::update(float dt) {
    // forzar BG silenciado cada frame — ningun thread puede restaurarlo
    ensureBgSilenced();
}

void LeaderboardHistoryLayer::ensureBgSilenced() {
...
bool LeaderboardHistoryLayer::ccMouseScroll(float x, float y) {
    ...
    CCSize contentSize = m_scrollView->getContainer()->getContentSize();
    ...
    m_scrollView->setContentOffset({offset.x, newY}, true);
    return true;
}
...
void LeaderboardHistoryLayer::loadHistory(std::string type) {
    ...
    for (auto& [k, v] : m_resolvedLevels) {
        if (v) v->release();
    }
    m_resolvedLevels.clear();
    m_pendingResolves = 0;
...
void LeaderboardHistoryLayer::createList() {
    ...
    auto scrollView = cocos2d::extension::CCScrollView::create();
    scrollView->setViewSize({listW, listH});
    ...
    scrollView->setContentOffset({0, listH - totalH});
    ...
    for (int p = 0; p < pageCount; p++) {
        ...
        if (localTex) {
            ...
            thumbPlaceholder->retain();
            ThumbnailLoader::get().requestLoad(levelID, fileName, [thumbPlaceholder, thumbSize](CCTexture2D* tex, bool) {
                geode::Loader::get()->queueInMainThread([thumbPlaceholder, tex, thumbSize] {
                    if (thumbPlaceholder->getParent() && tex) {
