...
void DynamicSongManager::resumeDynamicChannel() {
    ...
    // Restaurar volumen (el crossfade con ProfileMusic lo dejo en 0)
    auto engine = FMODAudioEngine::sharedEngine();
    float gameVolume = engine ? engine->m_musicVolume : m_bgVolumeBeforeFade;
    m_musicChannel->setVolume(gameVolume);
    ...
    log::info("[DynamicSong] Canal reanudado (vol:{:.2f})", gameVolume);
}
