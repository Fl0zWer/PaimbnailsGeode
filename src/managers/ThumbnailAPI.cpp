...
bool ThumbnailAPI::tryModCache(ModeratorCallback& callback) {
    ...
    if (std::chrono::duration_cast<std::chrono::seconds>(elapsed).count() < MOD_CACHE_TTL_SECONDS) {
        log::info("[ThumbnailAPI] mod check cache hit (mod={}, admin={})", m_modCache->isMod, m_modCache->isAdmin);
...
void ThumbnailAPI::updateModCache(bool isMod, bool isAdmin) {
    m_modCache = ModCacheEntry{isMod, isAdmin, std::chrono::steady_clock::now()};
}
...
void ThumbnailAPI::checkModerator(std::string const& username, ModeratorCallback callback) {
    ...
    // Cache: si tenemos resultado reciente, devolver inmediatamente
    ...
    // Saltar GDBrowser — ya tenemos el accountID del cliente (GJAccountManager).
    // El servidor hace su propia verificacion de identidad via API key + mod code.
    HttpClient::get().checkModeratorAccount(username, currentAccountID, [this, callback, username](bool isMod, bool isAdmin) {
        updateModCache(isMod, isAdmin);
        if (isAdmin) {
...
void ThumbnailAPI::checkModeratorAccount(std::string const& username, int accountID, ModeratorCallback callback) {
    ...
    // Saltar GDBrowser — ya tenemos el accountID del cliente.
    HttpClient::get().checkModeratorAccount(username, currentAccountID, [this, callback](bool isMod, bool isAdmin) {
        updateModCache(isMod, isAdmin);
        if (isAdmin) {
            Mod::get()->setSavedValue<bool>("is-verified-admin", true);
        }
...
void ThumbnailAPI::getTopCreators(ActionCallback callback) {
    ...
    HttpClient::get().getTopCreators([callback](bool success, std::string const& response) {
        callback(success, response);
    });
}
...
void ThumbnailAPI::getTopThumbnails(ActionCallback callback) {
    ...
    HttpClient::get().getTopThumbnails([callback](bool success, std::string const& response) {
        callback(success, response);
    });
}
