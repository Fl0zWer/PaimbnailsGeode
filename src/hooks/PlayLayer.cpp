...
static bool s_hideP1ForCapture = false;
static bool s_hideP2ForCapture = false;
...
class $modify(PaimonCapturePlayLayer, PlayLayer) {
        ...
        if (Mod::get()->getSettingValue<bool>("enable-thumbnail-taking")) {
            this->addEventListener(
                ...
                [this](Keybind const& keybind, bool down, bool repeat, double timestamp) {
                    ...
                    // Pausar musica INMEDIATAMENTE al presionar la tecla
                    // (antes de la captura, para que no se desincronice)
                    ...
                    FramebufferCapture::requestCapture(levelID, [safeRef, levelID](bool success, CCTexture2D* texture, std::shared_ptr<uint8_t> rgbaData, int width, int height) {
                        Loader::get()->queueInMainThread([safeRef, success, texture, rgbaData, width, height, levelID]() {
                            ...
                            if (!success || !texture || !rgbaData) {
                                ...
                                gCaptureInProgress.store(false);
                                // Reanudar musica si la captura fallo
                                if (auto* engine = FMODAudioEngine::sharedEngine()) {
                                    if (engine->m_backgroundMusicChannel) {
                                        engine->m_backgroundMusicChannel->setPaused(false);
                                    }
    ...
    void captureScreenshot(CapturePreviewPopup* existingPopup = nullptr) {
        ...
        // Uso Ref<> para gestion automatica del refcount — elimina riesgo de
        // leak si alguna rama no hace release, o double-free si dos ramas lo hacen.
        ...
        // Ref<> toma ownership del refcount=1 de new; se libera al salir de scope
        // o cuando los consumidores (popup/updateContent) hagan su propio retain.
        ...
        if (popup) { 
            if (existingPopup) {
                // popup ya existe -> esta ruta solo refresca el contenido sin recrearlo
            } else {
                popup->show(); 
            }
