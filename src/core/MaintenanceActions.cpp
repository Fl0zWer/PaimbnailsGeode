...
bool looksCorrupt(std::filesystem::path const& path, uintmax_t fileSize) {
    auto ext = toLower(geode::utils::string::pathToString(path.extension()));
    ...
    geode::listenForSettingChanges<bool>("maintenance-refresh-mod-code", +[](bool value) {
        ...
        ThumbnailAPI::get().checkModeratorAccount(username, accountID, [oldCode](bool isMod, bool isAdmin) {
            Loader::get()->queueInMainThread([oldCode, isMod, isAdmin]() {
                ...
                if (newCode.empty()) {
                    PaimonNotify::create("Verificado como mod/admin, pero el servidor no envio codigo.", NotificationIcon::Warning)->show();
                    return;
                }

                if (newCode != oldCode) {
                    PaimonNotify::create("Mod code actualizado correctamente.", NotificationIcon::Success)->show();
                } else {
