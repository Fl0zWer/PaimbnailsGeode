...
bool BanListPopup::init() {
    ...
    m_listMenu->setContentSize(m_scrollViewSize);
...
void BanListPopup::rebuildList(std::vector<std::string> const& users) {
    ...
    if (users.empty()) {
        ...
        m_scroll->m_contentLayer->setContentSize(viewSize);
    ...
    for (auto const& user : users) {
        ...
        cellContainer->setAnchorPoint({0.5f, 0.5f});
    ...
    if (totalH < viewSize.height) {
        totalH = viewSize.height;
    }
    
    m_listMenu->setContentSize({viewSize.width, totalH});
    m_listMenu->updateLayout();
    m_listMenu->setPosition({0, 0});
