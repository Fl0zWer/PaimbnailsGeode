...
void VerificationCenterLayer::showPreviewForItem(int index) {
    ...
    case PendingCategory::Verify:
        // Si el item tiene sugerencias con filename especifico, usar ese en vez del generico
        if (!item.suggestions.empty() && !item.suggestions[0].filename.empty()) {
            ThumbnailAPI::get().downloadSuggestionImage(item.suggestions[0].filename, onLoaded);
        } else {
            ThumbnailAPI::get().downloadSuggestion(itemID, onLoaded);
        }
        break;
    case PendingCategory::Banner:
        // Banners estan en pending_backgrounds/, usar filename directo del item
        if (!item.suggestions.empty() && !item.suggestions[0].filename.empty()) {
            ThumbnailAPI::get().downloadSuggestionImage(item.suggestions[0].filename, onLoaded);
        } else {
            // fallback: intentar con la URL de pending background
...
void VerificationCenterLayer::onClaimLevel(CCObject* sender) {
    ...
    ThumbnailAPI::get().checkModeratorAccount(username, accountID, [self, lvl, username, cat](bool isMod, bool isAdmin) {
        auto layer = self.lock();
        if (!layer) return;
